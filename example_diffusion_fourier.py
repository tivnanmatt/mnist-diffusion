...
if __name__ == '__main__':
    ...
    def modulation_transfer_function_func(t):
        ...
        LPF = fourier_transfer_function_LPF.repeat(t.shape[0], 1, 1, 1)
        BPF = fourier_transfer_function_BPF.repeat(t.shape[0], 1, 1, 1) * (0.5 + 0.5 * torch.exp(-5*_t*_t))
        HPF = fourier_transfer_function_HPF.repeat(t.shape[0], 1, 1, 1) * (0.1 + 0.9 * torch.exp(-5*_t*_t))
        return LPF + BPF + HPF

    def modulation_transfer_function_derivative_func(t):
        _t = t.reshape(-1, 1, 1, 1).repeat(1, 1, 28, 28)
        LPF = 0
        BPF = fourier_transfer_function_BPF.repeat(t.shape[0], 1, 1, 1) * 0.5 * (-10*_t * torch.exp(-5*_t*_t))
        HPF = fourier_transfer_function_HPF.repeat(t.shape[0], 1, 1, 1) * 0.9 * (-10*_t * torch.exp(-5*_t*_t))
        return LPF + BPF + HPF + 1e-10

    def noise_power_spectrum_func(t):
        _t = t.reshape(-1, 1, 1, 1).repeat(1, 1, 28, 28)
        LPF = fourier_transfer_function_LPF.repeat(t.shape[0], 1, 1, 1) * 0.02 * (1.0 - torch.exp(-10*_t*_t))
        BPF = fourier_transfer_function_BPF.repeat(t.shape[0], 1, 1, 1) * 0.06 * (1.0 - torch.exp(-10*_t*_t))
        HPF = fourier_transfer_function_HPF.repeat(t.shape[0], 1, 1, 1) * 0.04 * (1.0 - torch.exp(-10*_t*_t))
        return LPF + BPF + HPF + 1e-10

    def noise_power_spectrum_derivative_func(t):
        _t = t.reshape(-1, 1, 1, 1).repeat(1, 1, 28, 28)
        LPF = fourier_transfer_function_LPF.repeat(t.shape[0], 1, 1, 1) * 0.02 * (20*_t * torch.exp(-10*_t*_t))
        BPF = fourier_transfer_function_BPF.repeat(t.shape[0], 1, 1, 1) * 0.06 * (20*_t * torch.exp(-10*_t*_t))
        HPF = fourier_transfer_function_HPF.repeat(t.shape[0], 1, 1, 1) * 0.04 * (20*_t * torch.exp(-10*_t*_t))
        return LPF + BPF + HPF + 1e-10
    ...
    if loadPreviousWeights:
        if os.path.exists('./data/weights/diffusion_fourier.pt'):
            diffusion_model.load_state_dict(torch.load('./data/weights/diffusion_fourier.pt'))
            print('Loaded weights from ./data/weights/diffusion_fourier.pt')
    ...
    for epoch in range(100):
        ...
        if runReverseProcess:
            ...
            ani.save(f'./data/animations/diffusion_fourier.mp4', fps=15)

        torch.save(diffusion_model.state_dict(), './data/weights/diffusion_fourier.pt')